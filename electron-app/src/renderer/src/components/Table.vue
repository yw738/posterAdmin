<template>
  <section class="ces-table-page" v-if="isShow">
    ...
    <section class="ces-handle" v-if="isHandle && tableHandles.length">
      <template v-for="(item, index) in tableHandles" :key="index">
        <el-button
          ...
          :type="item.type"
          :icon="item.icon"
          :disabled="item.isDisabled && item.isDisabled(item, index)"
          v-if="
            item.type != 'uploadFile' && (!item.isShow || (item.isShow && item.isShow(item, index)))
          "
          @click="item.handle()"
    ...
    <section class="ces-table">
      ...
      >
        <el-table-column v-if="isSelection" type="selection" fixed :selectable="selectable" />
        <el-table-column v-if="isIndex" type="index" :label="indexLabel" width="60" />
        <!-- 数据栏 -->
        <el-table-column
        ...
        >
          <template #default="scope">
            ...
            <!-- 按钮 -->
            <span v-if="item.type === 'Button'" style="display: flex; flex-wrap: wrap">
              <el-button
                v-for="(btn, i) in item.btnList"
                v-has-perm="btn.roleId || 'noAuthentication'"
            ...
            <el-radio-group
              ...
              @change="item.change && item.change(scope.row)"
            >
              <el-radio v-for="(ra, raI) in item.radios" :label="ra.value" :key="raI">{{
                ra.label
              }}</el-radio>
            </el-radio-group>
            <!-- 复选框 -->
            <el-checkbox-group
              ...
              @change="item.change && item.change(scope.row)"
            >
              <el-checkbox v-for="(ra, raI) in item.checkboxs" :label="ra.value" :key="raI">{{
                ra.label
              }}</el-checkbox>
            </el-checkbox-group>
            <!-- 评价 -->
            ...
            <span v-if="item.type === 'Tooltip'">
              ...
              >
                ...
                >
                  ...
                  }}<el-icon style="margin-left: 2px" :size="14"><Warning /></el-icon
                ></span>
              </el-tooltip>
              <span
            ...
            <!-- 文字链接 -->
            <el-link
              v-if="item.type === 'TxtDown' && item.show && item.show(scope.row)"
              type="primary"
              @click="item.handle && item.handle(scope.row)"
              target="_blank"
              >{{ item.prop }}</el-link
    ...
    <section class="ces-pagination" v-if="isPagination">
      ...
      <el-pagination
        ...
        background
        :page-sizes="pagination.sizesArr || [15, 30, 50]"
...
export default {
  props: {
    ...
    cellStyleFn: { type: Function, default: () => () => {} }, // 单元格 css
  ...
  methods: {
    ...
    tableRowClassName({ row, rowIndex }) {
      ...
      if (row.QuoteStatus === 0) {
        return ''
      } else if (row.QuoteStatus === 1) {
        return 'warning-row'
      } else if (row.QuoteStatus === 2) {
        return 'primary-row'
      } else if (row.QuoteStatus === 3) {
        return 'success-row'
      } else if (row.QuoteStatus === 4) {
        return 'warning-row2'
      } else if (row.QuoteStatus === 5) {
        return 'primary-row2'
      }
...
/* .ces-table-page .el-button + .el-button {
  margin-left: 6px;
} */
.ces-table .el-button {
...
.el-table .el-table__row .el-table-column--selection
...
.el-table .primary-row2 {
  --el-table-tr-bg-color: #d1c4e9;
}

.ces-table thead .el-table-column--selection .cell {
  display: none;
}
</style>
